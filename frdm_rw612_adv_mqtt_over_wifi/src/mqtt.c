...
static int subscribe(struct mqtt_client *const c)
{
	struct mqtt_topic subscribe_topics[] = {
		...
		{
			.topic = {
				.utf8 = MQTT_SUB_PONG_TOPIC,
				.size = strlen(MQTT_SUB_PONG_TOPIC)
			},
			.qos = MQTT_QOS_1_AT_LEAST_ONCE
		}
	};
	...
	LOG_INF("Subscribing to: %s len %u", MQTT_SUB_TOPIC,
		(unsigned int)strlen(MQTT_SUB_TOPIC));
	...
	uint8_t *data, size_t len)
{
	...
	param.message.topic.topic.utf8 = MQTT_PUB_PING_TOPIC;
	param.message.topic.topic.size = strlen(MQTT_PUB_PING_TOPIC);
	...
	LOG_INF("to topic: %s len: %u",
		MQTT_PUB_PING_TOPIC,
		(unsigned int)strlen(MQTT_PUB_PING_TOPIC));
		      ...
		      const struct mqtt_evt *evt)
{
	...
	case MQTT_EVT_PUBLISH:
	{
		...
		if (err >= 0) {
			if(strcmp(p->message.topic.topic.utf8, MQTT_SUB_PONG_TOPIC) == 0) {
				data_print("Received: ", payload_buf, p->message.payload.len);
			} else if(strcmp(p->message.topic.topic.utf8, MQTT_SUB_TOPIC) == 0) {
				handle_payload(payload_buf, p->message.payload.len);
			}
...
static int cmd_mqtt_pingpong(const struct shell *sh, size_t argc, char *argv[])
{
	if(argc <= 1) {
        shell_print(sh, "Missing message argument, eg. mqtt echo <ping|hello>\n\r");
        return 1;
    } else {
		data_publish(MQTT_QOS_1_AT_LEAST_ONCE, argv[1], strlen(argv[1]));
	}
	return 0;
}

SHELL_STATIC_SUBCMD_SET_CREATE(mqtt_cmds,
    SHELL_CMD(echo, NULL, "Echo Ping pong message", cmd_mqtt_pingpong),
